...
/**
 * This class provides methods to work with impl.FTPServer. These are:
 *
...
public class FTPClient {
    ...
    /**
     * Creates new client, that connected to the impl.FTPServer.
     *
     ...
     * @throws IOException if any IOException occurred.
     */
    public List<FTPFile> list(@NotNull final String source) throws IOException {
        os.writeInt(1);
        os.writeUTF(source);
        os.flush();

        final List<FTPFile> list = new LinkedList<>();

        final int number = is.readInt();
        for (int i = 0; i < number; i++) {
            final String name = is.readUTF();
            final boolean isFolder = is.readBoolean();
            final FTPFile file = new FTPFile(name, isFolder);
            list.add(file);
        }
        return list;
    }
